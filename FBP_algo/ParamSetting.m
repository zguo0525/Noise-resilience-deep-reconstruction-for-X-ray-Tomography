...
param.nx = 150; % number of voxels
param.ny = 150;
param.nz = 1;

param.sx = 8; % mm (real size)
param.sy = 8; % mm
param.sz = 16/128; % mm
...
param.su = 25;	% mm (real size)
param.sv = 25/256;     % mm

% X-ray source and detector setting
param.DSD = 230.11+342.17;    %  Distance source to detector 
param.DSO = 230.11;	%  X-ray source to object axis distance

% angle setting
param.dir = 1;   % gantry rotating direction (clock wise/ counter clockwise)
param.dang = 1.8/2*10; % angular step size (deg)
param.deg = 0:param.dang:360; % you can change
